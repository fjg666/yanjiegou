...
class Tree {
	...
	* @return array
	*/
	public function get_child($myid,$pid='parentid'){
		$a = $newarr = array();
		if(is_array($this->arr)){
			foreach($this->arr as $id => $a){
	...
	* @return array
	*/
	public function get_all_childids($myid,$pid='parentid'){
		$child = array();
		$ids = array();

		$child = $this->get_child($myid,$pid);
		$total = !empty($child) ? count($child) : 0;
		if($total > 0){
			foreach($child as $id=>$value){
				$ids[] = $id;
				$ids = array_merge($ids,$this->get_all_childids($id,$pid));
			}
		}
		return $ids;
	}
