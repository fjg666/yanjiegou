...
class ShopAdmin extends Model
{
    ...
    public function login($data,$code='open'){
        ...
        $user=self::alias('a')
             ...
             ->field('a.pwd,a.admin_id,a.username,a.is_open,a.group_id,a.avatar,s.id,s.name,s.status,s.shoplogo,ag.rules,ag.is_super,s.is_lock shop_lock,s.shortname,s.bshopid') 
             ->find(); 
        if($user) {
            if ($user['shop_lock']==1) {
