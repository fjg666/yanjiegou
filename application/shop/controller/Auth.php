...
use think\Db;
use ensh\Leftnav;
use app\shop\model\ShopAdmin as Admin;
use app\shop\model\ShopAuthGroup as AuthGroup;
use app\shop\model\ShopAuthRule as authRule;
use think\request;
use think\Validate;
class Auth extends Common
{
    ...
    public function adminAdd(){
        if(Request::instance()->isAjax()){
            ...
            $auth_group = AuthGroup::where(['type'=>2,'shopid'=>SHID])->select();
    ...
    public function adminEdit(){
        if(request()->isPost()){
            ...
            if( $data['admin_id'] == session('aid')){
                session('sinfo.ursname',$data['username']);
                $avatar = $data['avatar']==''?'/static/admin/images/0.jpg':$data['avatar'];
                session('sinfo.avatar',$avatar);
            }
    ...
    public function groupAccess(){
        $nav = new Leftnav();
        $admin_rule=authRule::where(['type'=>2])->field('id,pid,title')->order('sort asc')->select();
        $rules = AuthGroup::where('group_id',input('id'))->value('rules');
    ...
    public function adminRule(){
        if(request()->isPost()){
            $arr = authRule::where(['type'=>2])->order('pid asc,sort asc')->select();
            foreach($arr as $k=>$v){
    ...
    public function clear(){
        // $arr = authRule::where(['pid'=>['neq',0],'type'=>2])->select();
        // foreach ($arr as $k=>$v){
        //     $p = authRule::where(['id'=>$v['pid'],'type'=>2])->find();
        //     if(!$p){
        //         authRule::where(['id'=>$v['id'],'type'=>2])->delete();
        //     }
        // }
        // cache('authRule', NULL);
        ...
        if(request()->isPost()){
            ...
            // if(!$arr){
                $authRule = authRule::all(function($query){
                    $query->order('sort', 'asc')->where('type',2);
                });
    ...
    public function ruleEdit(){
        if(request()->isPost()) {
            ...
            $admin_rule = authRule::get(function($query){
                $query->where(['id'=>input('id'),'type'=>2])->field('id,href,title,icon,sort,menustatus');
            });
